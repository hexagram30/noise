...
/// levels-forming curve.
///
/// This noise function maps the output value from the source function onto a
/// levels-forming curve. The start of the curve has a slode of zero; it's
...
pub struct Levels<'a, T> {
    ...
    /// Determines if the levels-forming curve between all control points is
    /// inverted.
    pub invert_levelss: bool,
...
impl<'a, T> Levels<'a, T> {
    pub fn new(source: &'a dyn NoiseFn<T>) -> Self {
        Levels {
            source,
            invert_levelss: false,
            control_points: Vec::with_capacity(2),
        }
    }

    /// Adds a control point to the levels-forming curve.
    ///
    /// Two or more control points define the levels-forming curve. The start
    ...
    pub fn add_control_point(mut self, control_point: f64) -> Self {
        ...
        // create new Levels with updated control_points vector
        Levels { ..self }
    }

    pub fn add_control_points(mut self, min: f64, max: f64, step: f64) -> Self {
        let control_points = FloatIterator::new_with_step(min, max, step);
...
impl<'a, T> NoiseFn<T> for Levels<'a, T> {
    fn get(&self, point: T) -> f64 {
        ...
        if self.invert_levelss {
            alpha = 1.0 - alpha;
            std::mem::swap(&mut input0, &mut input1);
        }

        // Squaring the alpha produces the levels effect.
        alpha *= alpha;

        // Now perform the cubic interpolation and return.
        linear_interpolate(input0, input1, alpha)
    }
}
